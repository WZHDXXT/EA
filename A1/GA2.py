...
def GA_1(problem: ioh.problem.PBO, pop_size, mutation_rate, crossover_rate) -> None:
    ...
    while problem.state.evaluations < budget:
        ...
        combined_population = np.vstack([parent, new_parent])  # 合并父代和子代
        combined_fitness = np.hstack([parent_f, new_parent_f])  # 合并适应度

    
        fitness_sort = np.argsort(combined_fitness)[::-1]  # 从高到低排序
        combined_population = combined_population[fitness_sort]
        combined_fitness = combined_fitness[fitness_sort]

        parent = combined_population[:pop_size]
        parent_f = combined_fitness[:pop_size]
    # problem.reset()
...
def studentnumber1_studentnumber2_GA(problem: ioh.problem.PBO) -> None:
    ...
    while problem.state.evaluations < budget:
        ...
        parent = combined_population[:GA_POP_SIZE]
        parent_f = combined_fitness[:GA_POP_SIZE]      
...
def create_problem(dimension: int, fid: int) -> Tuple[ioh.problem.PBO, ioh.logger.Analyzer]:
    ...
    l = logger.Analyzer(
        ...
        algorithm_name="(μ + λ) selection",  # name of your algorithm
